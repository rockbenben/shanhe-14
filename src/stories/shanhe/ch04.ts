import type { Chapter } from '../schema'

// 第四章《桥上的枪声》：1937 年 7 月，北平。
export const ch04: Chapter = {
  id: 'ch04',
  title: '桥上的枪声',
  era: '1937',
  epigraph: '从前只是一大块沃土，一大盘散沙的死中国，现在是有血有肉的活中国了。——朱自清《这一天》（1939）',
  historyNote:
    '1937 年 7 月 7 日夜，日军在北平西南卢沟桥一带演习，借口一名士兵失踪，要求进入宛平县城搜查，遭中国守军第二十九军拒绝后炮击宛平城，全面抗战由此开始。7 月 28 日，日军向南苑等地发起总攻；29 日，北平沦陷，前后不过二十一天。事变后大批学生、教员与报人辗转南下。本章时间线据当年《大公报》《世界日报》报道与平津流亡人士回忆，人物为虚构。',
  art: 'ch04.jpg',
  artCredit: '冲出宛平城的中国士兵，1937 年 7 月 · 佚名 · Wikimedia Commons · 公有领域',
  artExtra: [{ file: 'ch04-b.jpg', credit: '北平城内堆垒沙袋、备战日军进攻的第29军士兵，1937 年 7 月 · 佚名 · Wikimedia Commons · 公有领域'  }],
  entry: 'b1',
  beats: [
    {
      id: 'b1',
      photo: { file: 'ch04-f.jpg', credit: '前门（正阳门）大街街景，约 1920 年代 · 纽约公共图书馆(NYPL) · Wikimedia Commons · 公有领域（原为彩色网点明信片，已裁去白边题字并转灰度）——战前北平街市氛围照，非该场景实拍' },
      source: '七七前日军自五月起在宛平近郊频繁演习，由昼而夜、由虚弹而实弹，据宛平县长王冷斋《卢沟桥事变始末》',
      art: 'lg-card',
      narrative:
        '一年半以后，你领到了自己的记者证：一张硬纸卡，贴着照片，压着钢印，你的名字头一回端端正正印在上面。编辑先生说，往后跑外勤，腿要勤，眼要尖，笔要稳。这年七月的北平热得反常，槐花早谢尽了，蝉声把人叫得心里发空。西南郊的演习一天比一天勤，从白天演到深夜，先前放的是虚弹，近来听人说连实弹都用上了，报上的字眼却一天比一天含混。茶馆里老人摇着蒲扇说，不碍的，这些年哪一年不折腾。你把记者证收进内袋。天太热，硬纸卡贴着胸口，一会儿就焐热了。',
      next: 'b2',
    },
    {
      id: 'b2',
      photo: { file: 'ch04-e.jpg', credit: '二十九军在卢沟桥头向侵略者还击，1937 年 7 月 · 佚名／《中华》(上海)1939 年第 79 期 · Wikimedia Commons · 公有领域' },
      art: 'lg-wire',
      narrative:
        '七月八日天没亮，同事来砸你的门：出事了，卢沟桥。分社里灯火通明，电话铃一个压着一个。消息一条条对拢来：头天夜里，日军在卢沟桥边演习，说丢了一个兵，要进宛平城搜，守军没有应；后半夜枪响了，跟着是炮，宛平的城墙上落了弹。电讯纸摊满一桌，编辑先生的烟一支接一支。你抓起采访簿往城南跑。广安门一带已经戒严，只望得见西南方向的天，灰蒙蒙的，什么也看不出来。你在路边站了半晌，把看见的和没看见的，都记在本子上。',
      next: 'b2x',
    },
    {
      id: 'b2x',
      photo: { file: 'ch04-d.jpg', credit: '坚守卢沟桥的中国士兵，沙袋工事间可见桥栏石狮，1937 年 7 月 · 佚名／《中华》(上海)1937 年第 56 期 · Wikimedia Commons · 公有领域' },
      source: '赴长辛店、卢沟桥一线采访所见，据方大曾战地通讯《卢沟桥抗战记》（上海《世界知识》1937 年 8 月）',
      art: 'lg-wire',
      narrative:
        '过了两三天，你寻到个空子，搭运给养的卡车到了长辛店。同车有个背照相机的同行，比你还年轻，一路把镜头擦了又擦。铁路工人还守着机器房，指给你们看房顶上炸开的窟窿。再往前，卢沟桥就在望了：宛平的城墙上一片一片的弹痕，桥栏的石狮子缺了耳朵，守桥的兵背着大刀，就着城根吃干粮。带路的军官只许停一小会儿。回程的车上，那同行忽然说：这些要是不记下来，回到城里就没有人信。你把本子按在膝盖上，就着颠簸，一个字一个字地写。',
      next: 'b2y',
    },
    {
      id: 'b2y',
      source: '城内堆垒沙袋设防、七月下旬又连夜撤除，据当年平津报刊记载',
      photo: { file: 'ch04-b.jpg', credit: '北平城内堆垒沙袋、备战日军进攻的第29军士兵，1937 年 7 月 · 佚名 · Wikimedia Commons · 公有领域' },
      narrative:
        '城里也在变样。各处城门口垒起了沙袋，一层压一层，垛得齐人高，架枪的兵站在垛口后头，钢盔上罩着布。拉洋车的绕着走，卖冰核的照旧吆喝，只是嗓门低了。你把沙袋的层数、街口的岗数，一样一样记下来。到七月下旬的一天，忽然又传下话来：局面缓和了。当夜沙袋一垛一垛地撤，兵们不声不响地扛。第二天，戏园子照旧开了锣。你站在空出来的街口，看沙袋留下的一圈圈印子。心里那点不踏实，比看见沙袋的时候更重了。',
      next: 'b3',
    },
    {
      id: 'b3',
      photo: { file: 'ch04-g.jpg', credit: '北支事变（平津作战）中天津日租界内的巷战，1937 年 7 月 · 佚名（战前日本刊行照片）· Wikimedia Commons · 公有领域（PD-Japan-oldphoto）——同期平津战事实拍，非南苑实拍' },
      art: 'lg-stay',
      narrative:
        '半个月里，消息像潮水，一天三变：今天说谈妥了，明天城外又打起来。到七月末，南苑方向的炮响了一整天，城里谣言四起，车站挤满了人。报馆定了章程：分社南撤，愿走的即刻动身，路费报销。编辑先生问到你，你低头看自己那本采访簿，一半页码写的都是这座城的街名。走，是迟早的事；可怎么走，什么时候走，这一回轮到你自己定。',
      choices: [
        {
          text: '随报社南迁',
          to: 'b4',
          reaction:
            '你在名单上自己那一格里签了名。字签得很平常，你却看了两眼——六年前离家，是别人替你定的；这一笔，是你自己落的。',
        },
        {
          text: '留几日，把北平的最后模样写完',
          to: 'b4',
          reaction:
            '你朝先生摇了摇头：我押后几天。先生看了你一阵，没有劝，把一张空白路条压在你桌上：写完就走，别拖到走不成。你把采访簿翻到新的一页。',
        },
      ],
    },
    {
      id: 'b4',
      source: '沦陷后前门车站军警宪特密布，南下者一路上车下车反复被搜身盘检，据赵宝煦等流亡青年回忆（朱汉杰《全面抗战时期沦陷区青年奔赴后方的交通与体验》辑引）',
      art: 'lg-gate',
      narrative:
        '等你真正动身出城，已是八月初，城换了主人。前门车站外排着长队，卡口的木栅一层又一层，端枪的兵挨个翻检行李。墙上贴着七月末新出的告示，你没有走近去念，那股浆糊味你认得——六年前在沈阳，也是这个味道。前一夜你把记者证拆了，相片一处，硬纸一处，分开缝进衣角，针脚学的是母亲当年的手法。轮到你，兵翻了翻你的换洗衣裳，挥手放行。你走过木栅没有回头。城楼还是那座城楼，上头的旗，不是了。',
      next: 'b4x',
    },
    {
      id: 'b4x',
      art: 'lg-card',
      narrative:
        '南下的车要等到后半夜。站台上黑压压坐满了人，行李当凳子，孩子睡在包袱上，谁也不大说话。挨着你坐的是位老先生，守着两只捆麻绳的书箱，箱角磨得发白。他说他教了半辈子书，学校眼下是回不去了。灯一亮一灭，远处有巡查的皮靴声过去。老先生看了你半晌，忽然压低声音问：你是打城里出来的？城里如今，是个什么章程？这种问法你认得——五年前，上海弄堂口的裁缝问起关外，也是这么问的。',
      choices: [
        {
          text: '拣能说的，说给他听',
          to: 'b5',
          reaction:
            '你把这些天看见的拣了几件，声音放得很低。老先生听完，半天没有言语，只把书箱上的麻绳又紧了一道：书在，就还有的教。',
        },
        {
          text: '劝他先莫打听，上车要紧',
          to: 'b5',
          reaction:
            '你摇摇头：先生，上了车再说吧。他也不再问，从怀里摸出两块干饼，硬塞给你一块。两个人就着凉水，慢慢地吃了。',
        },
      ],
    },
    {
      id: 'b5',
      art: 'lg-train',
      narrative:
        '南下的车厢挤得落不下脚，行李架上坐着人，过道里睡着人。学生，教书先生，拖家带口的职员，各样口音说的是同一件事：不留下，不在那面旗底下过活。有人低声唱起歌，唱了两句自己停了，车轮声接了下去。你靠着车窗，看城墙退过去，郊野的青纱帐退过去。六年前那趟车，是家里人推着你走；这一趟，是你自己要走——走，不是逃，这两个字你在心里分得很清。车停了数不清多少回，让军车，让兵。天亮的时候，满车的人都醒着，谁也没有说话。',
      next: '$end',
    },
  ],
}
